...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s9 := Add(s8);
      var s10 := Push2(s9, 0x036e);
      var s11 := Swap2(s10);
      assert s11.Peek(2) == 0x36e;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s22 := Push2(s21, 0x0972);
      var s23 := JumpI(s22);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s22.stack[1] > 0 then
        ExecuteFromCFGNode_s123(s23, gas - 1)
      else
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s20 := Dup2(s19);
      var s21 := Swap1(s20);
      assert s21.Peek(3) == 0x402;
      assert s21.Peek(8) == 0x147;
      var s22 := Push2(s21, 0x07b9);
      var s23 := Jump(s22);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s142(s23, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s11 := Dup6(s10);
      assert s11.Peek(8) == 0x36e;
      assert s11.Peek(13) == 0x402;
      assert s11.Peek(18) == 0x147;
      var s12 := Add(s11);
      var s13 := MStore(s12);
      var s14 := Push1(s13, 0x00);
      //  Go to the next instruction at pc + 1
  ...
  function {:opaque} {:verify true} ExecuteFromCFGNode_s145(s0: EState, gas: nat): (s': EState)
    ...
    requires s0.stack[11] == 0x402

    requires s0.stack[16] == 0x147
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s21 := Pop(s20);
      assert s21.Peek(5) == 0x36e;
      assert s21.Peek(10) == 0x402;
      assert s21.Peek(15) == 0x147;
      var s22 := Pop(s21);
      var s23 := Pop(s22);
      var s24 := Swap3(s23);
      var s25 := Swap2(s24);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s27 := SLoad(s26);
      var s28 := Push2(s27, 0x044c);
      var s29 := Swap1(s28);
      var s30 := Dup3(s29);
      var s31 := Push2(s30, 0x07e5);
      assert s31.Peek(0) == 0x7e5;
      assert s31.Peek(3) == 0x44c;
      assert s31.Peek(8) == 0x147;
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s151(s32, gas - 1)
  }
