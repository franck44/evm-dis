...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := And(s9);
      var s11 := Dup2(s10);
      assert s11.Peek(4) == 0xc66;
      assert s11.Peek(8) == 0x26f;
      assert s11.Peek(9) == 0x1b0;
      var s12 := Eq(s11);
      var s13 := Push2(s12, 0x0bd0);
      var s14 := JumpI(s13);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s2 := Swap2(s1);
      var s3 := Pop(s2);
      var s4 := Push2(s3, 0x0c95);
      var s5 := Push1(s4, 0x20);
      var s6 := Dup5(s5);
      var s7 := Add(s6);
      var s8 := Push2(s7, 0x0bba);
      var s9 := Jump(s8);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s9 := SLt(s8);
      var s10 := IsZero(s9);
      var s11 := Push2(s10, 0x0c0f);
      assert s11.Peek(0) == 0xc0f;
