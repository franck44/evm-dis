...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := And(s9);
      var s11 := Dup2(s10);
      assert s11.Peek(4) == 0x810;
      assert s11.Peek(9) == 0x19b;
      assert s11.Peek(10) == 0xf3;
      var s12 := Eq(s11);
      var s13 := Push2(s12, 0x0767);
      var s14 := JumpI(s13);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s31 := Add(s30);
      assert s31.Peek(4) == 0x265;
      assert s31.Peek(8) == 0xdf;
      var s32 := Push2(s31, 0x02fd);
      var s33 := Jump(s32);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s26 := MLoad(s25);
      var s27 := Push2(s26, 0x06f4);
      var s28 := Swap2(s27);
      var s29 := Dup2(s28);
      var s30 := MStore(s29);
      var s31 := Push1(s30, 0x20);
      assert s31.Peek(2) == 0x6f4;
      assert s31.Peek(10) == 0x265;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s33 := Lt(s32);
      var s34 := IsZero(s33);
      var s35 := Push2(s34, 0x03eb);
      var s36 := JumpI(s35);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s35.stack[1] > 0 then
        ExecuteFromCFGNode_s66(s36, gas - 1)
      else
        ExecuteFromCFGNode_s64(s36, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s30 := Push1(s29, 0x84);
      var s31 := Add(s30);
      assert s31.Peek(4) == 0x3f8;
      assert s31.Peek(9) == 0xdf;
      var s32 := Push2(s31, 0x02fd);
      var s33 := Jump(s32);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s69(s33, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s29 := Dup1(s28);
      var s30 := SLoad(s29);
      var s31 := Push2(s30, 0x0201);
      assert s31.Peek(0) == 0x201;
      assert s31.Peek(8) == 0xb6;
      var s32 := Swap1(s31);
      var s33 := Push2(s32, 0x0827);
      var s34 := Jump(s33);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s31.Peek(5) == 0xdf;
      var s32 := Swap2(s31);
      var s33 := Push2(s32, 0x0265);
      var s34 := Swap2(s33);
      var s35 := Dup6(s34);
      var s36 := Swap1(s35);
      var s37 := Push2(s36, 0x0355);
      var s38 := Swap1(s37);
      var s39 := Dup7(s38);
      var s40 := Swap1(s39);
      var s41 := Push2(s40, 0x0861);
      //  Go to the next instruction at pc + 1
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s33 := Jump(s32);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s153(s33, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s33 := Lt(s32);
      var s34 := IsZero(s33);
      var s35 := Push2(s34, 0x0306);
      var s36 := JumpI(s35);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s35.stack[1] > 0 then
        ExecuteFromCFGNode_s169(s36, gas - 1)
      else
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s31 := Add(s30);
      //  Go to the next instruction at pc + 1
      ExecuteFromCFGNode_s168(s31, gas - 1)
  }
