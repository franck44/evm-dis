...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s20 := Gt(s19);
      var s21 := Or(s20);
      assert s21.Peek(11) == 0x179;
      assert s21.Peek(12) == 0x17e;
      var s22 := IsZero(s21);
      var s23 := Push2(s22, 0x0954);
      var s24 := JumpI(s23);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s23.stack[1] > 0 then
        ExecuteFromCFGNode_s105(s24, gas - 1)
      else
  ...
  function {:opaque} {:verify true} ExecuteFromCFGNode_s111(s0: EState, gas: nat): (s': EState)
    ...
    requires s0.stack[12] == 0x179

    requires s0.stack[13] == 0x17e
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s23 := Jump(s22);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s126(s23, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s31.Peek(8) == 0x17e;
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s135(s32, gas - 1)
  }
