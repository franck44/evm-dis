...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x0899);
      assert s1.Peek(0) == 0x899;
      assert s1.Peek(8) == 0x1d9;
      assert s1.Peek(9) == 0x1de;
      var s2 := Push2(s1, 0x0838);
      var s3 := Jump(s2);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s22 := IsZero(s21);
      var s23 := Push2(s22, 0x08be);
      var s24 := JumpI(s23);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s23.stack[1] > 0 then
        ExecuteFromCFGNode_s24(s24, gas - 1)
      else
        ExecuteFromCFGNode_s22(s24, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x08be);
      assert s1.Peek(0) == 0x8be;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x08f1);
      assert s1.Peek(0) == 0x8f1;
      assert s1.Peek(10) == 0x1d9;
      assert s1.Peek(11) == 0x1de;
      var s2 := Dup6(s1);
