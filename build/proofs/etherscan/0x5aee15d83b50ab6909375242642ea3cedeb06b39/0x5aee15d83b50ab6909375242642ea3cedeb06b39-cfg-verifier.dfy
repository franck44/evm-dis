...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := And(s9);
      var s11 := Dup2(s10);
      assert s11.Peek(4) == 0x75b;
      assert s11.Peek(8) == 0x1d8;
      assert s11.Peek(9) == 0x15a;
      var s12 := Eq(s11);
      var s13 := Push2(s12, 0x06dc);
      var s14 := JumpI(s13);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s9 := SLt(s8);
      var s10 := IsZero(s9);
      var s11 := Push2(s10, 0x071b);
      assert s11.Peek(0) == 0x71b;
