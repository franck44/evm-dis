...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s11.Peek(7) == 0x6cb;
      assert s11.Peek(11) == 0x29a;
      assert s11.Peek(15) == 0xf9;
      var s12 := Swap1(s11);
      var s13 := Push2(s12, 0x0786);
      var s14 := Jump(s13);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s31.Peek(8) == 0xf9;
      var s32 := SLoad(s31);
      var s33 := Push2(s32, 0x06f9);
      var s34 := Swap1(s33);
      var s35 := Dup3(s34);
      var s36 := Push2(s35, 0x0774);
      var s37 := Jump(s36);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x08af);
      assert s1.Peek(0) == 0x8af;
      assert s1.Peek(8) == 0x151;
      assert s1.Peek(9) == 0x156;
      var s2 := Push2(s1, 0x084e);
      var s3 := Jump(s2);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s24 := Add(s23);
      //  Go to the next instruction at pc + 1
      ExecuteFromCFGNode_s127(s24, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s29 := Swap1(s28);
      var s30 := Dup3(s29);
      var s31 := Push2(s30, 0x0774);
      assert s31.Peek(0) == 0x774;
      assert s31.Peek(3) == 0x43b;
      assert s31.Peek(8) == 0x156;
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s144(s32, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s9 := SLt(s8);
      var s10 := IsZero(s9);
      var s11 := Push2(s10, 0x0827);
      assert s11.Peek(0) == 0x827;
