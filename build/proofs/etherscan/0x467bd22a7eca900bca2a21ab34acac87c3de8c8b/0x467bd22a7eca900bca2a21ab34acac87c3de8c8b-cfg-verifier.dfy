...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s9 := Add(s8);
      var s10 := Push2(s9, 0x037d);
      var s11 := Swap2(s10);
      assert s11.Peek(2) == 0x37d;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Add(s9);
      var s11 := SLt(s10);
      assert s11.Peek(7) == 0x1b3;
      assert s11.Peek(8) == 0x1b8;
      var s12 := Push2(s11, 0x0938);
      var s13 := JumpI(s12);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x094a);
      assert s1.Peek(0) == 0x94a;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Push1(s9, 0x24);
      var s11 := Push1(s10, 0x00);
      assert s11.Peek(2) == 0x94a;
      assert s11.Peek(10) == 0x1b3;
      assert s11.Peek(11) == 0x1b8;
      var s12 := Revert(s11);
      // Segment is terminal (Revert, Stop or Return)
      s12
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Dup4(s9);
      var s11 := Add(s10);
      assert s11.Peek(11) == 0x1b3;
      assert s11.Peek(12) == 0x1b8;
      var s12 := And(s11);
      var s13 := Dup2(s12);
      var s14 := Add(s13);
      var s15 := Dup2(s14);
      ...
      var s20 := Gt(s19);
      var s21 := Or(s20);
      assert s21.Peek(11) == 0x1b3;
      assert s21.Peek(12) == 0x1b8;
      var s22 := IsZero(s21);
      var s23 := Push2(s22, 0x096f);
      var s24 := JumpI(s23);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Push1(s9, 0x24);
      var s11 := Push1(s10, 0x00);
      assert s11.Peek(2) == 0x96f;
      assert s11.Peek(13) == 0x1b3;
      assert s11.Peek(14) == 0x1b8;
      var s12 := Revert(s11);
      // Segment is terminal (Revert, Stop or Return)
      s12
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s20 := Gt(s19);
      var s21 := IsZero(s20);
      assert s21.Peek(10) == 0x1b3;
      assert s21.Peek(11) == 0x1b8;
      var s22 := Push2(s21, 0x098d);
      var s23 := JumpI(s22);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x09a3);
      assert s1.Peek(0) == 0x9a3;
      assert s1.Peek(10) == 0x1b3;
      assert s1.Peek(11) == 0x1b8;
      var s2 := Dup6(s1);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s11.Peek(14) == 0x1b3;
      assert s11.Peek(15) == 0x1b8;
      var s12 := Eq(s11);
      var s13 := Push2(s12, 0x085d);
      var s14 := JumpI(s13);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s1.Peek(12) == 0x1b3;
      assert s1.Peek(13) == 0x1b8;
      var s2 := Swap2(s1);
      var s3 := Swap1(s2);
      var s4 := Pop(s3);
      var s5 := Jump(s4);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Swap4(s9);
      var s11 := MStore(s10);
      assert s11.Peek(6) == 0x1b8;
      var s12 := Swap1(s11);
      var s13 := Swap3(s12);
      var s14 := Pop(s13);
      var s15 := Swap1(s14);
      var s16 := Push2(s15, 0x0411);
      var s17 := Swap1(s16);
      var s18 := Dup3(s17);
      var s19 := Swap1(s18);
      var s20 := Dup2(s19);
      var s21 := Swap1(s20);
      assert s21.Peek(3) == 0x411;
      assert s21.Peek(8) == 0x1b8;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s11 := Dup6(s10);
      assert s11.Peek(8) == 0x37d;
      assert s11.Peek(13) == 0x411;
      assert s11.Peek(18) == 0x1b8;
      var s12 := Add(s11);
      var s13 := MStore(s12);
      var s14 := Push1(s13, 0x00);
      //  Go to the next instruction at pc + 1
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := JumpDest(s0);
      assert s1.Peek(6) == 0x37d;
      assert s1.Peek(11) == 0x411;
      assert s1.Peek(16) == 0x1b8;
      var s2 := Dup2(s1);
      var s3 := Dup2(s2);
      var s4 := Lt(s3);
      var s5 := IsZero(s4);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Dup6(s0);
      assert s1.Peek(7) == 0x37d;
      assert s1.Peek(12) == 0x411;
      assert s1.Peek(17) == 0x1b8;
      var s2 := Dup2(s1);
      var s3 := Add(s2);
      var s4 := Dup4(s3);
      var s5 := Add(s4);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s11 := Push1(s10, 0x1f);
      assert s11.Peek(7) == 0x37d;
      assert s11.Peek(12) == 0x411;
      assert s11.Peek(17) == 0x1b8;
      var s12 := Not(s11);
      var s13 := Push1(s12, 0x1f);
      var s14 := Dup4(s13);
      var s15 := Add(s14);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s27 := SLoad(s26);
      var s28 := Push2(s27, 0x045b);
      var s29 := Swap1(s28);
      var s30 := Dup3(s29);
      var s31 := Push2(s30, 0x07e5);
      assert s31.Peek(0) == 0x7e5;
      assert s31.Peek(3) == 0x45b;
      assert s31.Peek(8) == 0x1b8;
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s12 := Swap1(s11);
      var s13 := Pop(s12);
      var s14 := Push2(s13, 0x0492);
      var s15 := Dup2(s14);
      var s16 := Push2(s15, 0x0a57);
      var s17 := Jump(s16);
      //  JUMP to the target at Peek(0)
