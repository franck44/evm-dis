...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s21.Peek(12) == 0x18b;
      var s22 := IsZero(s21);
      var s23 := Push2(s22, 0x088d);
      var s24 := JumpI(s23);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s23.stack[1] > 0 then
        ExecuteFromCFGNode_s100(s24, gas - 1)
      else
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s31.Peek(8) == 0x18b;
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s130(s32, gas - 1)
  }
