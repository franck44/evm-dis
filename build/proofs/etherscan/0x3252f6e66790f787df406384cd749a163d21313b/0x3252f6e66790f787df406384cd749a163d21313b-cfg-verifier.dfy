...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x0797);
      assert s1.Peek(0) == 0x797;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s32 := SLoad(s31);
      var s33 := Push2(s32, 0x0747);
      var s34 := Swap1(s33);
      var s35 := Dup3(s34);
      var s36 := Push2(s35, 0x07c2);
      var s37 := Jump(s36);
      //  JUMP to the target at Peek(0)
  ...
  function {:opaque} {:verify true} ExecuteFromCFGNode_s52(s0: EState, gas: nat): (s': EState)
    ...
    requires s0.stack[3] == 0x7cd

    requires s0.stack[7] == 0x747
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s11.Peek(8) == 0x141;
      var s12 := Push2(s11, 0x08eb);
      var s13 := JumpI(s12);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x08fd);
      assert s1.Peek(0) == 0x8fd;
      assert s1.Peek(8) == 0x13c;
      assert s1.Peek(9) == 0x141;
  ...
  function {:opaque} {:verify true} ExecuteFromCFGNode_s116(s0: EState, gas: nat): (s': EState)
    ...
    requires s0.stack[0] == 0x8fd

    requires s0.stack[8] == 0x13c

    requires s0.stack[9] == 0x141
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s11 := Push0(s10);
      assert s11.Peek(2) == 0x8fd;
      assert s11.Peek(10) == 0x13c;
      assert s11.Peek(11) == 0x141;
      var s12 := Revert(s11);
      // Segment is terminal (Revert, Stop or Return)
      s12
  }
  ...
  function {:opaque} {:verify true} ExecuteFromCFGNode_s118(s0: EState, gas: nat): (s': EState)
    ...
    requires s0.stack[11] == 0x141

    decreases gas
  {
    if gas == 0 then s0
    else
      var s1 := Push2(s0, 0x0922);
      assert s1.Peek(0) == 0x922;
      assert s1.Peek(11) == 0x13c;
      assert s1.Peek(12) == 0x141;
      var s2 := Push2(s1, 0x089c);
      var s3 := Jump(s2);
      //  JUMP to the target at Peek(0)
  ...
  function {:opaque} {:verify true} ExecuteFromCFGNode_s119(s0: EState, gas: nat): (s': EState)
    ...
    requires s0.stack[0] == 0x922

    requires s0.stack[11] == 0x13c

    requires s0.stack[12] == 0x141
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Push1(s9, 0x24);
      var s11 := Push0(s10);
      assert s11.Peek(2) == 0x922;
      assert s11.Peek(13) == 0x13c;
      assert s11.Peek(14) == 0x141;
      var s12 := Revert(s11);
      // Segment is terminal (Revert, Stop or Return)
      s12
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s21.Peek(11) == 0x141;
      var s22 := Push2(s21, 0x093f);
      var s23 := JumpI(s22);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s22.stack[1] > 0 then
        ExecuteFromCFGNode_s122(s23, gas - 1)
      else
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x0955);
      assert s1.Peek(0) == 0x955;
      assert s1.Peek(10) == 0x13c;
      assert s1.Peek(11) == 0x141;
      var s2 := Dup6(s1);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := And(s9);
      var s11 := Dup2(s10);
      assert s11.Peek(4) == 0x955;
      assert s11.Peek(14) == 0x13c;
      assert s11.Peek(15) == 0x141;
      var s12 := Eq(s11);
      var s13 := Push2(s12, 0x0836);
      var s14 := JumpI(s13);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push0(s0);
      assert s1.Peek(3) == 0x955;
      assert s1.Peek(13) == 0x13c;
      assert s1.Peek(14) == 0x141;
      var s2 := Dup1(s1);
      var s3 := Revert(s2);
      // Segment is terminal (Revert, Stop or Return)
      s3
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s11.Peek(10) == 0x141;
      var s12 := Push2(s11, 0x0944);
      var s13 := Jump(s12);
      //  JUMP to the target at Peek(0)
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s20 := Dup3(s19);
      var s21 := Add(s20);
      assert s21.Peek(4) == 0x141;
      var s22 := MStore(s21);
      var s23 := Push1(s22, 0x64);
      var s24 := Add(s23);
      //  Go to the next instruction at pc + 1
      ExecuteFromCFGNode_s133(s24, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      assert s21.Peek(8) == 0x141;
      var s22 := Push2(s21, 0x0797);
      var s23 := Jump(s22);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s141(s23, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s29 := Swap1(s28);
      var s30 := Dup3(s29);
      var s31 := Push2(s30, 0x07c2);
      assert s31.Peek(0) == 0x7c2;
      assert s31.Peek(3) == 0x43b;
      assert s31.Peek(8) == 0x141;
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s150(s32, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s9 := SLt(s8);
      var s10 := IsZero(s9);
      var s11 := Push2(s10, 0x0875);
      assert s11.Peek(0) == 0x875;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s31 := Jump(s30);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s174(s31, gas - 1)
  }
