...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Add(s9);
      var s11 := SLt(s10);
      assert s11.Peek(7) == 0x1a3;
      assert s11.Peek(8) == 0x1a8;
      var s12 := Push2(s11, 0x0898);
      var s13 := JumpI(s12);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Push1(s9, 0x24);
      var s11 := Push1(s10, 0x00);
      assert s11.Peek(2) == 0x8aa;
      assert s11.Peek(10) == 0x1a3;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s20 := Gt(s19);
      var s21 := Or(s20);
      assert s21.Peek(11) == 0x1a3;
      assert s21.Peek(12) == 0x1a8;
      var s22 := IsZero(s21);
      var s23 := Push2(s22, 0x08cf);
      var s24 := JumpI(s23);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s23.stack[1] > 0 then
        ExecuteFromCFGNode_s74(s24, gas - 1)
      else
        ExecuteFromCFGNode_s72(s24, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x08cf);
      assert s1.Peek(0) == 0x8cf;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := Push1(s9, 0x24);
      var s11 := Push1(s10, 0x00);
      assert s11.Peek(2) == 0x8cf;
      assert s11.Peek(13) == 0x1a3;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s20 := Gt(s19);
      var s21 := IsZero(s20);
      assert s21.Peek(10) == 0x1a3;
      assert s21.Peek(11) == 0x1a8;
      var s22 := Push2(s21, 0x08ed);
      var s23 := JumpI(s22);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x0903);
      assert s1.Peek(0) == 0x903;
      assert s1.Peek(10) == 0x1a3;
      assert s1.Peek(11) == 0x1a8;
      var s2 := Dup6(s1);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s10 := And(s9);
      var s11 := Dup2(s10);
      assert s11.Peek(4) == 0x903;
      assert s11.Peek(14) == 0x1a3;
      assert s11.Peek(15) == 0x1a8;
      var s12 := Eq(s11);
      var s13 := Push2(s12, 0x07bd);
      var s14 := JumpI(s13);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s26 := Add(s25);
      //  Go to the next instruction at pc + 1
      ExecuteFromCFGNode_s87(s26, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x0384);
      assert s1.Peek(0) == 0x384;
      assert s1.Peek(6) == 0x1a8;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s16 := Pop(s15);
      var s17 := Swap1(s16);
      var s18 := Push2(s17, 0x03d7);
      var s19 := Swap1(s18);
      var s20 := Dup3(s19);
      var s21 := Swap1(s20);
      assert s21.Peek(2) == 0x3d7;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s21 := Pop(s20);
      assert s21.Peek(5) == 0x35b;
      assert s21.Peek(10) == 0x3d7;
      assert s21.Peek(15) == 0x1a8;
      var s22 := Pop(s21);
      var s23 := Pop(s22);
      var s24 := Swap3(s23);
      var s25 := Swap2(s24);
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s27 := SLoad(s26);
      var s28 := Push2(s27, 0x0421);
      var s29 := Swap1(s28);
      var s30 := Dup3(s29);
      var s31 := Push2(s30, 0x0745);
      assert s31.Peek(0) == 0x745;
      assert s31.Peek(3) == 0x421;
      assert s31.Peek(8) == 0x1a8;
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s104(s32, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s34 := Jump(s33);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s130(s34, gas - 1)
  }
