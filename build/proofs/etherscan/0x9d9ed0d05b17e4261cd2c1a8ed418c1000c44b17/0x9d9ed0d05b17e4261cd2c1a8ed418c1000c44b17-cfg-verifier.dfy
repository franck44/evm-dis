...
module  {:disableNonlinearArithmetic} EVMProofObject {
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x092f);
      assert s1.Peek(0) == 0x92f;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s22 := IsZero(s21);
      var s23 := Push2(s22, 0x0954);
      var s24 := JumpI(s23);
      // This is a JUMPI segment, determine next pc using second top-most element of stack
      if s23.stack[1] > 0 then
        ExecuteFromCFGNode_s73(s24, gas - 1)
      else
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s1 := Push2(s0, 0x0954);
      assert s1.Peek(0) == 0x954;
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s23 := Jump(s22);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s94(s23, gas - 1)
  }
    ...
    decreases gas
  {
    if gas == 0 then s0
      ...
      var s32 := Jump(s31);
      //  JUMP to the target at Peek(0)
      ExecuteFromCFGNode_s103(s32, gas - 1)
  }
