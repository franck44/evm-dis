...
include "./simpleCall.dfy" 
import opened DafnyEVMProofObject
// const JUMPDESTS := [ 0x7, 0x10, 0x32, 0x53]


method {:verify false} Test1(s0: EvmState.ExecutingState)
...
method {:verify true} Test2(s0: EvmState.ExecutingState)
  requires ExecuteFromTag_0.requires(s0)
...
function {:verify false} R1(s0: EvmState.ExecutingState): EvmState.State
  requires ExecuteFromTag_0.requires(s0)
...
function {:verify false} R2(s0: EvmState.ExecutingState): EvmState.State
  requires ExecuteFromTag_0.requires(s0)
...
function {:verify false} R3(s0: EvmState.ExecutingState): EvmState.State
  requires ExecuteFromTag_0.requires(s0)
