...
class default__:
    ...
    def Main(args):
        if (len(args)) < (2):
            ...
            _dafny.print((_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "OK0"))).VerbatimString(False))
            _dafny.print((_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "\n"))).VerbatimString(False))
    ...
    def PrintInstructions(s):
        while True:
            with _dafny.label():
                if (len(s)) == (0):
                    _dafny.print((_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "OK1"))).VerbatimString(False))
                    _dafny.print((_dafny.SeqWithoutIsStrInference(map(_dafny.CodePoint, "\n"))).VerbatimString(False))
                elif True:
